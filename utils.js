...
function getBitsetArray(bits, num) {
    var rets = [];
    for (var i = 1; i < num; ++i) {
        var shiftNum = 1 << i;
        if (shiftNum & bits) {
...
module.exports.getBitsetArray = getBitsetArray;
...
var bits = getBitsetArray(255, 8);
var test = {};
bits.map(function(item) {
    test[item] = {
        'a': item * 2,
        'b': item * 3
    };
});
console.log("getBitsetArray =", test);